/**
 * LightTunnel — Vanilla JavaScript / ES Module implementation
 * Based on React Bits LightTunnel component (powered by OGL)
 */
...
/**
 * Initialize LightTunnel in a DOM container
 * @param {HTMLElement} container
 * @param {object} [options]
 * @param {object} [oglModule] - Optional OGL instance or will load from global/CDN
 */
export async function createLightTunnel(container, options = {}, oglModule = null) {
  if (!container) throw new Error('LightTunnel: container element is required');

  let OGL = oglModule || window.OGL;
  if (!OGL) {
    try {
      OGL = await import('https://cdn.jsdelivr.net/npm/ogl@1.0.11/dist/ogl.mjs');
    } catch {
  ...
  container.classList.add('light-tunnel-container');
  ...
  const updateUniforms = (newOpts) => {
    Object.assign(opts, newOpts);
    const u = program.uniforms;
    u.uSpeed.value = opts.speed;
    u.uFlowDir.value = opts.flowDirection === 'outward' ? -1.0 : 1.0;
    u.uPulseSpeed.value = opts.pulseSpeed;
    u.uPulseLength.value = opts.pulseLength;
    u.uPulseBlend.value = opts.pulseBlend;
    u.uPulseWidth.value = opts.pulseWidth;
    u.uCableCount.value = opts.cableCount;
    u.uThickness.value = opts.thickness;
    u.uRimWidth.value = opts.rimWidth;
    u.uWaviness.value = opts.waviness;
    u.uSway.value = opts.sway;
    u.uSize.value = opts.size;

    const center = u.uCenter.value;
    center[0] = opts.centerX;
    center[1] = opts.centerY;

    u.uGlow.value = opts.glow;
    u.uFadeNear.value = opts.fadeNear;
    u.uFadeFar.value = opts.fadeFar;
    u.uBrightness.value = opts.brightness;
    u.uColorVariance.value = opts.colorVariance ? 1.0 : 0.0;
    u.uGrain.value = opts.grain ? 1.0 : 0.0;
    u.uGrainIntensity.value = opts.grainIntensity;
    u.uOpacity.value = opts.opacity;

    const cable = hexToRgb(opts.cableColor);
    const cableU = u.uCableColor.value;
    cableU[0] = cable[0];
    cableU[1] = cable[1];
    cableU[2] = cable[2];

    const pulse = hexToRgb(opts.pulseColor);
    const pulseU = u.uPulseColor.value;
    pulseU[0] = pulse[0];
    pulseU[1] = pulse[1];
    pulseU[2] = pulse[2];

    const tunnel = hexToRgb(opts.tunnelColor);
    const tunnelU = u.uTunnelColor.value;
    tunnelU[0] = tunnel[0];
    tunnelU[1] = tunnel[1];
    tunnelU[2] = tunnel[2];

    u.uTunnelOpacity.value = opts.tunnelOpacity;
  };
  ...
  const loop = (t) => {
    ...
    off[0] = (currentMouse[0] - 0.5) * opts.mouseStrength;
    off[1] = (currentMouse[1] - 0.5) * opts.mouseStrength;
...
export default createLightTunnel;
